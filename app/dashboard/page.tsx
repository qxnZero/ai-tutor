        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {courses.length === 0 ? "No courses yet" : `${courses.length} courses created`}
            </p>
          </CardContent>
        </Card>
        <Card>
            ...
            <div className="text-2xl font-bold">
              {coursesWithProgress
                .reduce((acc, course) => {
                  ...
                  return acc + course._count.lessons * 30 * (course.progress / 100)
                }, 0)
                .toFixed(0)}{" "}
              min
            </div>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {coursesWithProgress.length > 0
                ? `${(coursesWithProgress.reduce((acc, course) => acc + course.progress, 0) / coursesWithProgress.length).toFixed(0)}%`
                : "0%"}
            </div>
            ...
            {coursesWithProgress.length === 0 ? (
              <Card>
                <CardContent className="py-8 text-center">
                  ...
                  <Button asChild>
                    <Link href="/#generate-course">Create Your First Course</Link>
                  </Button>
                </CardContent>
              </Card>
            ) : (
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {course._count.lessons} lessons • {course.progress}% complete
                          </p>
                        </div>
                  ...
                  <Card key={note.id}>
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <Button size="sm" variant="outline" asChild>
                          <Link href={`/courses/${note.lesson.module.courseId}/${note.lessonId}`}>View</Link>
                        </Button>
                      </div>
            ...
            {bookmarks.length === 0 ? (
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Quick access to your saved lessons</CardDescription>
                </CardHeader>
                <CardContent className="p-0">
                  <div className="divide-y">
                    {bookmarks.map((bookmark) => (
                      <div key={bookmark.id} className="p-4 hover:bg-muted/50">
                        <Link
                        ...
                        >
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <h4 className="font-medium">{bookmark.lesson.title}</h4>
                              <p className="text-sm text-muted-foreground">{bookmark.lesson.module.course.title}</p>
                            </div>
                          </div>
                        </Link>
                      </div>
